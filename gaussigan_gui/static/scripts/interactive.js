...
document.getElementById("theta-range").oninput = function() {
    updateTheta(document.getElementById("theta-range").value * Math.PI / 180.0)
};
document.getElementById("theta-num").oninput = function() {
    updateTheta(document.getElementById("theta-num").value * Math.PI / 180.0)
};
...
var cur_interact_mode = interact_modes.TRANS_XY;
...
function calcArcballVector(x, y) {
    ...
    arc_vec = math.multiply(arc_vec, math.rotationMatrix(camera_theta, [0., 1., 0.]));
    ...
    function dragging() {
        ...
        switch(cur_interact_mode) {
            case interact_modes.TRANS_XY:
                d3.event.subject.mu3d[0] += scaled_dx * Math.cos(camera_theta);
                d3.event.subject.mu3d[2] -= scaled_dx * Math.sin(camera_theta);
                d3.event.subject.mu3d[1] += scaled_dy;
                break;
            case interact_modes.TRANS_Z:
                d3.event.subject.mu3d[0] += scaled_dy * Math.sin(camera_theta);
                d3.event.subject.mu3d[2] += scaled_dy * Math.cos(camera_theta);
                break;
...
function calculateProjection() {
    ...
    gl.uniform1f(rot_y_attrib, camera_theta);
...
async function fetchLandmarks(fetch_mask=false) {
    ...
    camera_theta = json_response.theta3d;

    updateTheta(camera_theta);
    calculateProjection();
